/**
 * Novel Editor Demo - Runtime Bundler Loader
 *
 * Loads Novel (Notion-style WYSIWYG editor) and compiles it at runtime
...
const IMPORT_MAP = {
  imports: {
    ...
    // Novel - bundle all dependencies together
    'novel': 'https://esm.sh/novel@1.0.2?external=react,react-dom&bundle-deps',
  }
};
  ...
  try {
    ...
    const bundler = createBundler({
      fetcher: async (url) => {
        ...
        // Handle CSS module imports - return empty style object
        if (url.includes('.css') || url.includes('.module.css')) {
          console.warn(`CSS module, returning empty module: ${url}`);
          return { contents: 'export default {};' };
        }

        const response = await fetch(url);
        if (!response.ok) {
          // Handle any remaining 404s for react-tweet gracefully
          if (url.includes('react-tweet')) {
            console.warn(`react-tweet resource not found, returning empty module: ${url}`);
            return { contents: 'export default {};' };
          }
    ...
    updateProgress(65, 'Bundling Novel editor (this may take a moment)...');
    ...
    updateProgress(95, 'Starting Novel editor...');
